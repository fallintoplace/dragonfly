...
void RdbSerializer::MaybeCompressLegacyBuf(FlushState flush_state) {
  const bool is_last_chunk = flush_state == FlushState::kFlushEndEntry;
  const bool should_compress_chunk = is_last_chunk && !entry_was_split_;
  const bool is_compression_enabled = compression_mode_ == CompressionMode::MULTI_ENTRY_ZSTD ||
                                      compression_mode_ == CompressionMode::MULTI_ENTRY_LZ4;
  VLOG(2) << "PrepareFlush:" << is_last_chunk << " split=" << entry_was_split_;
  if (should_compress_chunk && is_compression_enabled)
    CompressBlob();
...
io::Bytes RdbSerializer::PrepareFlush(FlushState flush_state) {
  auto* mem_buf = mem_buf_controller_.CurrentBuffer();
  if (mem_buf->InputLen() == 0)
    return {};

  if (!mem_buf_controller_.TagEntriesEnabled())
    MaybeCompressLegacyBuf(flush_state);
...
string RdbSerializer::Flush(FlushState flush_state) {
  ...
  // For tagged entry, compress the entire blob as a whole
  if (mem_buf_controller_.TagEntriesEnabled())
